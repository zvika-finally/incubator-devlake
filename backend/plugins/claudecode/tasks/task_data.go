...
func DecodeAndValidateTaskOptions(options map[string]interface{}) (*ClaudeCodeOptions, errors.Error) {
	var op ClaudeCodeOptions
	err := helper.Decode(options, &op, nil)
	if err != nil {
		return nil, errors.Default.Wrap(err, "error decoding claudecode task options")
	}
	if op.DaysBack == 0 {
