...
func CollectSkillUsage(taskCtx plugin.SubTaskContext) errors.Error {
	...
	if strings.TrimSpace(connection.Organization) == "" {
		taskCtx.GetLogger().Info("No organization configured, skipping skill usage collection")
		return nil
	}
	...
	err = collector.InitCollector(helper.ApiCollectorArgs{
		...
		PageSize:              1,
		Incremental:           true,
		UrlTemplate:           "v1/organizations/analytics/skills",
